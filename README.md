...
- `--output_video_path`: Path to the output video (e.g., `./out/vid_1_splatting.mp4`).
...
- `--input_video_path`: Path to the splatting video result generated by the first stage (e.g., `./out/vid_1_splatting.mp4`).
- `--save_dir`: Directory for the output stereo video (e.g., `./out`).
...
```bash
./1_infer.sh ./in/vid.mp4
```

The Python stage can be disabled with `--enabled False`:

```bash
python s1_pipeline/s3_greenscreen.py --input_video_path ./out/vid_2_sbs.mp4 \
                         --output_video_path ./out/vid_3_greenscreen.mp4 \
                         --depth_npz_path ./out/vid_1_splatting.npz \
                         --enabled True
```
...
```bash
python s1_pipeline/s4_interpolation.py --input_video_path ./out/vid_3_greenscreen.mp4 \
                                  --output_video_path ./out/vid_4_interp.mp4 \
                                  --target_fps 45
```
...
```bash
python s1_pipeline/s5_upscale.py --input_video_path ./out/vid_4_interp.mp4 \
                             --output_video_path ./out/vid_5_upscale.mp4
```
...
```bash
python s1_pipeline/s6_green_cleanup.py --input_video_path ./out/vid_5_upscale.mp4 \
                           --output_video_path ./out/vid_6_result.mp4 \
                           --enabled True
```
...
Each step prints its duration and average/max resource usage. `1_infer.sh` also prints the full convert duration.
